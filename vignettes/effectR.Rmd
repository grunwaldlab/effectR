...
MAFFT is a multiple sequence alignment program that uses Fourier-transform algorithms to align multiple sequences. We recommend downloading and installing MAFFT by following the instructions and steps in the [MAFFT installation](http://mafft.cbrc.jp/alignment/software/) web site. Make sure that you remember the directory in which `MAFFT` is installed, of if the installation is sucessful, make sure to obtain the path via bash/tsh/console:
...
For more information about HMMR go to the HMMR website: http://hmmer.org/
...
We have created a `ORF` object that includes the list of translated ORF's from the subset of XX ORF's from the *P. infestans* genome. For more information on the `SeqFastadna` objects please read the [`seqinr manual`](https://cran.r-project.org/web/packages/seqinr/seqinr.pdf).
...
Here we show an example to search for sequences with RxLR-EER motifs from the XX ORF subset of *P. infestans*.
This ORF example data set contains 17 sequences with RxLR-EER motifs and XX sequences with the LFLAK motifs found in CRN effectors. 
...
We observe that the `REGEX` object has 17 sequences with the RxLR motif. These sequences will be aligned using `MAFFT`, and used to build a HMM profile to search for similar sequences.
...
The `hmm.search` function has resulted in 27 effector candidates. 
